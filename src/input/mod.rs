// 这个文件是Niri窗口管理器的输入事件处理核心模块，主要负责处理所有输入设备事件（键盘、鼠标、触摸板、触摸屏、平板笔等）并转化为窗口管理操作。以下是它的核心功能概述：
// 核心功能

//     输入事件路由：

//         处理键盘按键、鼠标移动/点击、触摸事件、手势事件、平板笔事件等

//         通过process_input_event()统一分发到具体处理函数

//     快捷键系统：

//         解析配置的快捷键绑定（Binds）

//         实现按键拦截逻辑（should_intercept_key()）

//         支持按键重复（Repeat）和冷却时间（Cooldown）

//     窗口操作：

//         窗口移动/调整大小（MoveGrab/ResizeGrab）
...
//         热角（Hot Corner）触发概览模式

//         触摸屏手势

// 关键组件

//     设备管理：跟踪输入设备状态（on_device_added/removed）

//     光标控制：自动隐藏/显示逻辑（hide_cursor_if_needed）

//     状态同步：处理键盘LED状态、快捷键抑制状态

//     平板支持：专业级平板笔事件处理（压力感应/倾斜检测）

// 架构特点

//     模块化设计：通过子模块处理特定功能（如move_grab处理窗口拖动）

//     Wayland协议集成：与Smithay输入栈深度集成
